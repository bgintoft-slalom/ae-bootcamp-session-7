...
The todo-service is a stateless full-stack task management application: a React frontend backed by a Node.js/Express REST API for CRUD operations on todos. The frontend and backend will run as containers on AWS ECS Fargate behind an Application Load Balancer, with networking, CloudWatch logs, and Container Insights providing the hosting and observability foundation.
